...
## Install in Firefox for local development
...
Firefox temporary add-ons are removed when Firefox restarts, so load `dist-firefox/manifest.json` again after restarting the browser. The Firefox build uses a Manifest V2 background script because some Firefox versions still disable Manifest V3 background service workers for temporary add-ons. Bookmark items may show letter placeholders instead of favicons because Firefox does not support Chrome's `_favicon` extension endpoint. Release update badges are disabled in the Firefox development build because its generated manifest intentionally differs from the Chromium build.
...
- `scripts/build.mjs` bundles TypeScript entry points and copies the extension's static and update-tracking assets into `dist` or `dist-firefox`.
...
- `dist` is generated by `npm run build` and is the directory loaded by Edge or other Chromium browsers.
- `dist-firefox` is generated by `npm run build:firefox` and is the directory loaded by Firefox.
